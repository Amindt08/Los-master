...
const TambahDeskripsi = () => {
    const [DeskripsiMkLite, setDeskripsiMkLite] = useState([]);
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const deskripsiMkLiteResponse = await axios.get(API_ENDPOINTS.GETDESKRIPSIMKLITE);
            setDeskripsiMkLite(deskripsiMkLiteResponse.data);
        } catch (error) {
    ...
    const handleAdd = async (Keterangan: string) => {
        try {
            await axios.post(API_ENDPOINTS.TAMBAHDESKRIPSIMKLITE, { Keterangan });
    ...
    const handleUpdate = async (Kode: string, Keterangan: string) => {
        try {
            await axios.put(API_ENDPOINTS.UPDATEDESKRIPSIMKLITE(Kode), { Keterangan });
    ...
    const handleDelete = async (Kode: string) => {
        try {
            await axios.delete(API_ENDPOINTS.DELETEDESKRIPSIMKLITE(Kode));
