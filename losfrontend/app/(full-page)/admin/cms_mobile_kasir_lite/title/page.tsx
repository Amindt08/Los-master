...
const TambahTitle = () => {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const titleResponse = await axios.get(API_ENDPOINTS.GETTITLEMKLITE);
            setModel(titleResponse.data);
        } catch (error) {
    ...
    const handleAdd = async (Keterangan: string) => {
        try {
            await axios.post(API_ENDPOINTS.TAMBAHTITLEMKLITE, { Keterangan });
    ...
    const handleUpdate = async (Kode: string, Keterangan: string) => {
        try {
            await axios.put(API_ENDPOINTS.UPDATETITLEMKLITE(Kode), { Keterangan });
    ...
    const handleDelete = async (Kode: string) => {
        try {
            await axios.delete(API_ENDPOINTS.DELETETITLEMKLITE(Kode));
