...
const TambahDeskripsi = () => {
    const [DeskripsiCustomer, setDeskripsiCustomer] = useState([]);
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const deskripsiCustomerResponse = await axios.get(API_ENDPOINTS.GETDESKRIPSICUSTOMER);
            setDeskripsiCustomer(deskripsiCustomerResponse.data);
        } catch (error) {
    ...
    const handleAdd = async (Keterangan: string) => {
        try {
            await axios.post(API_ENDPOINTS.TAMBAHDESKRIPSICUSTOMER, { Keterangan });
    ...
    const handleUpdate = async (Kode: string, Keterangan: string) => {
        try {
            await axios.put(API_ENDPOINTS.UPDATEDESKRIPSICUSTOMER(Kode), { Keterangan });
    ...
    const handleDelete = async (Kode: string) => {
        try {
            await axios.delete(API_ENDPOINTS.DELETEDESKRIPSICUSTOMER(Kode));
