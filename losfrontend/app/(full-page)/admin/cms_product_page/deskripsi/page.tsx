...
const TambahDeskripsi = () => {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const deskripsiProductResponse = await axios.get(API_ENDPOINTS.GETDESKRIPSIPRODUCT);
            setDeskripsiProduct(deskripsiProductResponse.data);
        } catch (error) {
    ...
    const handleAdd = async (Keterangan: string) => {
        try {
            await axios.post(API_ENDPOINTS.TAMBAHDESKRIPSIPRODUCT, { Keterangan });
    ...
    const handleUpdate = async (Kode: string, Keterangan: string) => {
        try {
            await axios.put(API_ENDPOINTS.UPDATEDESKRIPSIPRODUCT(Kode), { Keterangan });
    ...
    const handleDelete = async (Kode: string) => {
        try {
            await axios.delete(API_ENDPOINTS.DELETEDESKRIPSIPRODUCT(Kode));
