...
const TambahSidebarProduct = () => {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const sidebarProductResponse = await axios.get(API_ENDPOINTS.GETSIDEBARPRODUCT);
            setSidebarProduct(sidebarProductResponse.data);
        } catch (error) {
    ...
    const handleAdd = async (Keterangan: string) => {
        try {
            await axios.post(API_ENDPOINTS.TAMBAHSIDEBARPRODUCT, { Keterangan });
    ...
    const handleUpdate = async (Kode: string, Keterangan: string) => {
        try {
            await axios.put(API_ENDPOINTS.UPDATESIDEBARPRODUCT(Kode), { Keterangan });
    ...
    const handleDelete = async (Kode: string) => {
        try {
            await axios.delete(API_ENDPOINTS.DELETESIDEBARPRODUCT(Kode));
        ...
        <>
            ...
            {isLoading ? (
                ...
                <DataTableWithCRUD
                    ...
                    imageInput={true}
                    columns={[
                        { field: 'Keterangan', header: 'Sidebar' }
                    ]}
                    onAdd={handleAdd}
...
export default TambahSidebarProduct;