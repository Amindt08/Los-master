...
const TambahNegara = () => {
    ...
    const handleAdd = async (Keterangan: string) => {
        try {
            await axios.post(API_ENDPOINTS.TAMBAHNEGARA, { Keterangan });
            toast.current?.show({ severity: 'success', summary: 'Success', detail: 'Negara berhasil ditambahkan', life: 3000 });
            fetchData();
        } catch (error) {
    ...
    const handleUpdate = async (Kode: string, Keterangan: string) => {
        try {
            await axios.put(API_ENDPOINTS.UPDATENEGARA(Kode), { Keterangan });
            toast.current?.show({ severity: 'success', summary: 'Success', detail: 'Negara berhasil diupdate', life: 3000 });
            fetchData();
        } catch (error) {
    ...
    const handleDelete = async (Kode: string) => {
        try {
            await axios.delete(API_ENDPOINTS.DELETENEGARA(Kode));
